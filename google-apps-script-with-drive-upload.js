...
function doPost(e) {
  try {
            ...
            if (part.includes('Content-Type:')) {
              ...
              if (filenameMatch && filenameMatch[1]) {
                ...
                if (dataStart < dataEnd) {
                  ...
                  Logger.log('Found file: ' + fieldName + ' = ' + filename);
                }
              }
            } else {
              ...
              if (valueStart < valueEnd) {
                ...
                Logger.log('Found field: ' + fieldName + ' = ' + value);
              }
            }
          }
        }
    ...
    try {
      // Handle photo upload
      if (files.photo && files.photo.data) {
        const photoBlob = Utilities.newBlob(files.photo.data, files.photo.contentType, files.photo.name);
      ...
      if (files.resume && files.resume.data) {
        const resumeBlob = Utilities.newBlob(files.resume.data, files.resume.contentType, files.resume.name);
